...
from utils.ssim_utils import msssim
...
class WeightedSSIMGuidance(Guidance):
    def load_transmission(self, transmission):
        self.density = transmission.detach()
    ...
    def _forward(self, target_x0: torch.Tensor, pred_x0: torch.Tensor, t: int) -> Tuple[torch.Tensor, float]:
        ...
        with torch.enable_grad():
            pred_x0.requires_grad_(True)
            loss = -msssim(pred_x0 * .5 + .5, target_x0 * .5 + .5, normalize=True)
        scale = self.scale
        g = -torch.autograd.grad(loss, pred_x0)[0] * self.density * scale
        return g, loss.item()
